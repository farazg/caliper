...
function demoInit() {
    ...
    fs.writeFileSync("backlog_data.json", '[');
}
...
function demoRefreshData(updates) {
    ...
    let unfinished = demoData.summary.txSub - demoData.summary.txSucc - demoData.summary.txFail;
    fs.appendFileSync("backlog_data.json", JSON.stringify({ round: demoData.summary.round + 1, timestamp: Date.now(), unfinished: unfinished }) + ',\n');
}
...
function demoStopWatch(output) {
    ...
    update();
    let fs = require('fs');
    fs.appendFileSync("backlog_data.json", ']');
}
