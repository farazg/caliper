    ...
    async _submitSingleTransaction(context, invokeSettings, timeout) {
        ...
        try {
            ...
            try {
                ...
                invokeStatus.SetStatusEndorsed(Date.now());
            } catch (err) {
                invokeStatus.Set('time_endorse', Date.now());
                invokeStatus.time_endorse = Date.now();
                invokeStatus.SetStatusEndorsed(Date.now());
            ...
            invokeStatus.time_orderer_ack = Date.now();
            invokeStatus.SetStatusOrdererAck(Date.now());
        ...
        fs.appendFileSync('results_all.json', JSON.stringify(invokeStatus)+',\n'); 
